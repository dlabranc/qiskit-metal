# -*- coding: utf-8 -*-
"""
Transmon Pocket 6 with rounded corners and half circles near junction.
...
Adds half circles with smooth filleted connection close to the junction.
...
import numpy as np
from shapely.geometry import box as _box, Point, Polygon
from shapely.ops import unary_union
from qiskit_metal import draw, Dict
from qiskit_metal.qlibrary.core import BaseQubit

# Check the stem and fillet for smooth transition
...
class TransmonPocketH6(BaseQubit):
    """Transmon pocket with 6 connection pads + optional rounded corners + half circles near junction."""
    ...
    default_options = Dict(
        ...
        # --- NEW: half circles near junction ---
        half_circle_radius="15um",     # radius of the half circles
        half_circle_offset="0um",      # distance from junction edge to circle center
        stem_width="10um", # width of rectangular stem connecting to pad
        stem_height="20um", # height of rectangular stem
        stem_junction_width="5um", # width of rectangular stem connecting to pad
        stem_junction_height="3um", # width of rectangular stem connecting to pad
        fillet="3um",      # fillet radius for smooth transition
        sub_fillet="3um",      # fillet radius for smooth transition in the junction area
    ...
    TOOLTIP = "Transmon pocket with 6 connection pads + rounded corners + half circles near junction."
    ...
    def make_pocket(self):
        ...
        # Create half circles with stems near the junction
        hc_radius = float(p.half_circle_radius)
        hc_offset = float(p.half_circle_offset)
        stem_width = float(p.stem_width)
        stem_height = float(p.stem_height)
        j_stem_width = float(p.stem_junction_width)
        j_stem_height = float(p.stem_junction_height)
        fillet = float(p.fillet)
        sub_fillet = float(p.sub_fillet)

        
        # Half circle for top pad (protruding downward toward junction)
        center_y_top = pad_gap / 2 + hc_offset
        half_circle_top = Point(0, center_y_top).buffer(hc_radius)
        # Clip to only keep the lower half (toward junction)
        clip_box_top = _box(-hc_radius, center_y_top - hc_radius, hc_radius, center_y_top)
        half_circle_top = half_circle_top.intersection(clip_box_top)
        
        # Add rectangular stem connecting to pad
        stem_top = Polygon([(-stem_width/2, center_y_top), 
                            (-stem_width/2, center_y_top - hc_radius - stem_height), 
                            (stem_width/2, center_y_top - hc_radius - stem_height), 
                            (stem_width/2, center_y_top)])
        
        sub_stem_top = Polygon([(-stem_width/2, center_y_top),
                                (-stem_width/2, center_y_top - hc_radius - stem_height), 
                                (-j_stem_width/2, center_y_top - hc_radius - stem_height - j_stem_height), 
                                (+j_stem_width/2, center_y_top - hc_radius - stem_height - j_stem_height),
                                (stem_width/2, center_y_top - hc_radius - stem_height),
                                (stem_width/2, center_y_top),])
        
        # Union and apply fillet for smooth transition
        half_circle_assembly_top = unary_union([half_circle_top, stem_top])

        if fillet > 0:
            # Smooth the connection with buffer operations
            half_circle_assembly_top = half_circle_assembly_top.buffer(fillet, join_style=1).buffer(-fillet, join_style=1)

        # Half circle for bot pad (protruding downward toward junction)
        center_y_bot = -pad_gap / 2 - hc_offset
        half_circle_bot = Point(0, center_y_bot).buffer(hc_radius)
        # Clip to only keep the lower half (toward junction)
        clip_box_bot = _box(-hc_radius, center_y_bot + hc_radius, hc_radius, center_y_bot)
        half_circle_bot = half_circle_bot.intersection(clip_box_bot)
        
        # Add rectangular stem connecting to pad
        stem_bot = Polygon([(-stem_width/2, center_y_bot), 
                            (-stem_width/2, center_y_bot + hc_radius + stem_height), 
                            (stem_width/2, center_y_bot + hc_radius + stem_height), 
                            (stem_width/2, center_y_bot)])
        
        sub_stem_bot = Polygon([(-stem_width/2, center_y_bot),
                                (-stem_width/2, center_y_bot + hc_radius + stem_height), 
                                (-j_stem_width/2, center_y_bot + hc_radius + stem_height + j_stem_height), 
                                (+j_stem_width/2, center_y_bot + hc_radius + stem_height + j_stem_height),
                                (stem_width/2, center_y_bot + hc_radius + stem_height),
                                (stem_width/2, center_y_bot)])
        
        # Union and apply fillet for smooth transition
        half_circle_assembly_bot = unary_union([half_circle_bot, stem_bot])

        if fillet > 0:
            # Smooth the connection with buffer operations
            half_circle_assembly_bot = half_circle_assembly_bot.buffer(fillet, join_style=1).buffer(-fillet, join_style=1)
        
        
        # Combine pads with half circle assemblies
        pad_top = unary_union([
            draw.translate(pad, 0, +(pad_height + pad_gap) / 2.0),
            half_circle_assembly_top
        ])

        if fillet > 0:
            # Smooth the connection with buffer operations
            pad_top = pad_top.buffer(fillet, join_style=1).buffer(-fillet, join_style=1)

        pad_bot = unary_union([
            draw.translate(pad, 0, -(pad_height + pad_gap) / 2.0),
            half_circle_assembly_bot
        ])

        if fillet > 0:
            # Smooth the connection with buffer operations
            pad_bot = pad_bot.buffer(fillet, join_style=1).buffer(-fillet, join_style=1)

        # To avoid fillet in the junction area
    ...
    def make_connection_pad(self, name: str):
        ...
        if loc_W != 0:
            ...
            connector_pad = _rounded_rectangle(
                ...
                xoff=-pad_width / 2, yoff=+pad_height,
                radius=cn_r
            )
        ...
        if pad_x_abs is not None:
            x_off = loc_W*(p.pocket_width/2 - (pad_x_abs + pad_width/2))
        else:
            x_off = loc_W * (p.pad_width) / 2.0
